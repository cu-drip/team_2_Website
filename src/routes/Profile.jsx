...
function renderUserFields(user) {
  ...
  return Object.entries(user).map(([key, value]) => (
    <Box key={key} sx={{ display: 'flex', mb: 1, flexWrap: 'wrap' }}>
      <Typography sx={{ fontWeight: 600, minWidth: 100, color: 'primary.main', mr: 1 }}>
        {key}:
      </Typography>
                ...
                <Box component="span" sx={{ ml: 1 }}>
                  ...
                  {value.map((v, i) => (
                    <Box component="span" key={i} sx={{ ml: 0.5 }}>
                      {typeof v === 'object' ? JSON.stringify(v, null, 2) : String(v)}{i < value.length - 1 ? ',' : ''}
                    </Box>
                  ))}
                  ]
                </Box>
    ...
    return (
        ...
        >
          ...
          >
            ...
            <Box sx={{ textAlign: 'left', bgcolor: 'background.default', borderRadius: 1, p: 2, fontSize: { xs: '0.9rem', sm: '1rem' }, overflowX: 'auto' }}>
