...
export default function AdminUserFeedbacks() {
  ...
  return (
    <Box sx={{ 
      ...
      >
        {userInfo?.avatarUrl && (
          <Avatar src={userInfo.avatarUrl} alt={userInfo?.name || "Аватар"} sx={{ width: 40, height: 40 }} />
        )}
      ...
      >
        ...
        >
          <Tab 
            label={`Отзывы о матчах (${matchTotalElements})`}
            sx={{ color: 'text.secondary' }}
          />
          <Tab 
            label={`Отзывы о турнирах (${tournamentTotalElements})`}
            sx={{ color: 'text.secondary' }}
          />
        </Tabs>
            ...
            <Box>
              ...
              {matchTotalPages > 1 && (
                <Box sx={{ display: "flex", justifyContent: "center", mt: 3 }}>
                  <Pagination
                    count={matchTotalPages}
                    page={matchCurrentPage + 1}
